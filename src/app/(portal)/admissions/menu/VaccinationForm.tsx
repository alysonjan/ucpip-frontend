...
import { vaccinationFormSchema, VaccinationFormSchema } from "@/types/vaccinationForm";
...
export const VaccinationForm: React.FC<VaccinationFormProps> = ({ data, onContinue, setActiveTab }) => {
  // const isVaccinationOnly = data.services.length === 1 && data.services.includes("vaccination");
  ...
  useEffect(() => {
    ...
    const currentServiceIndex = sortedServices.findIndex((service) => service === "vaccination");
    setIsLastService(currentServiceIndex + 1 >= sortedServices.length);
  }, [data.services]);
  const form = useForm<VaccinationFormSchema>({
  ...
  const onSubmit = async (formData: z.infer<typeof vaccinationFormSchema>) => {
    const payload = {
      consultation_service: "vaccination",
      ...data,
      ...formData,
    };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 flex flex-col w-full">
        <div className="flex flex-col items-center space-y-10 p-16 bg-gray-100 rounded-lg shadow-md w-[800px] mx-auto">
          <h2 className="text-2xl font-semibold text-gray-700">Vaccination Form</h2>
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormLabel className="font-bold text-gray-600">
                  Vaccination Given <span className="text-red-500">*</span>
                </FormLabel>
                <FormControl>
                  <Input
                    {...field}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormLabel className="font-bold text-gray-600">
                  Dose No. <span className="text-red-500">*</span>
                </FormLabel>
                <FormControl>
                  <Input
                    {...field}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                <FormLabel className="font-bold text-gray-600">
                  Purpose of Vaccination <span className="text-red-500">*</span>
                </FormLabel>
                <FormControl>
                  <Textarea
                    {...field}
