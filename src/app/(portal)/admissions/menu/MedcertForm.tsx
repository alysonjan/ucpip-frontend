...
import { MedicalCertFormSchema, medicalCertFormSchema } from "@/types/medcertForm";

type MedcertFormProps = {
...
export const MedcertForm: React.FC<MedcertFormProps> = ({ data, onContinue, setActiveTab }) => {
  ...
  useEffect(() => {
    ...
    const currentServiceIndex = sortedServices.findIndex((service) => service === "med_certificate");
    setIsLastService(currentServiceIndex + 1 >= sortedServices.length);
  }, [data.services]);
  const form = useForm<MedicalCertFormSchema>({
    resolver: zodResolver(medicalCertFormSchema),
    defaultValues: {
      medcert_data: "",
      medcert_remarks: "",
    },
  ...
  const onSubmit = async (formData: z.infer<typeof medicalCertFormSchema>) => {
    const payload = {
      consultation_service: "med_certificate",
      ...data,
      ...formData,
    };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 flex flex-col w-full">
        <div className="flex flex-col items-center space-y-10 p-16 bg-gray-100 rounded-lg shadow-md w-[800px] mx-auto">
          <h2 className="text-2xl font-semibold text-gray-700">Medical Certificate</h2>
